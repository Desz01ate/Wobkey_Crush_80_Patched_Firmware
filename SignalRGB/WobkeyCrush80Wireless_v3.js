 ...
 * Requires firmware/firmware_per_key_v2.bin (PKRG protocol version 1).
 ...
 * Esc=0, F1=1, Caps Lock=52 were confirmed over wired USB. See docs/PER-KEY-RGB.md.
 */
export function Name() { return "Wobkey Crush 80 (Wireless) V3 Per-Key"; }
...
export function Initialize() {
    ...
    try {
        ...
        device.log(`Wobkey Wireless V3 initialization failed: ${error.message}`);
        try { restoreState(); }
        catch (restoreError) { device.log(`Wobkey Wireless V3 restore failed: ${restoreError.message}`); }
    }
}
...
export function Render() {
    ...
    try {
        ...
        streaming = false;
        device.log(`Wobkey Wireless V3 streaming stopped: ${error.message}. Toggle streaming to reconnect.`);
        try { restoreState(); }
        catch (restoreError) { device.log(`Wobkey Wireless V3 restore failed: ${restoreError.message}`); }
    }
}
...
export function Shutdown() {
    ...
    try {
        ...
        device.log(`Wobkey Wireless V3 shutdown restore failed: ${error.message}`);
    }
}
