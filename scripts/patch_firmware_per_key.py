...
See docs/PER-KEY-RGB.md. This builder never opens a device or flashes firmware.
"""
import argparse
import binascii
import hashlib
...
def build_image(source):
    if len(source) != IMAGE_SIZE or hashlib.sha256(source).hexdigest() != BASE_SHA256:
        raise ValueError("Input must be the exact v1.06 firmware/v2_patched.bin baseline")
...
def main():
    ...
    parser.add_argument("--input", type=Path, default=ROOT / "firmware/v2_patched.bin")
    parser.add_argument("--output", type=Path, default=ROOT / "firmware/firmware_per_key_v2.bin")
    parser.add_argument("--ota-input", type=Path, default=ROOT / "firmware/code_2M_v2_patched.bin")
    parser.add_argument("--ota-output", type=Path, default=ROOT / "firmware/code_2M_per_key_v2.bin")
