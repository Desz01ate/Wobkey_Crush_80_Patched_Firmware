...
The optimized v2 capability handshake was physically observed in the user's Windows sample, but the previous SDK rejected v2 before any lighting writes. The offline compatibility fix does **not** verify Windows sample writes or restoration; those still require a newly authorized hardware smoke rerun before Windows SDK hardware support can be claimed.
