...
The user physically observed the optimized v2 capability handshake on Windows (response `CH8AAFBLUkcCXAgACQsAAAAAAAAAAAAAAAAAAAAAAAA=`). The prior SDK rejected its protocol byte 2 before lighting writes. This compatibility change is verified offline; **Windows sample writes, readback, and restoration on v2 still require a renewed authorized hardware smoke run** before claiming Windows SDK hardware support.
