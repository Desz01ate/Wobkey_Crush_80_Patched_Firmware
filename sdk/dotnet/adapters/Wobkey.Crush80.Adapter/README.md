...
For hardware-free integration, `OpenAsync(IHidTransport)` accepts an injected SDK transport.
The repository's [`emulator/`](../../../../emulator/README.md) project uses this overload to
run the same grid, apply, lease, and restoration path while displaying the resulting RGB frame
in a local browser.
